...
import {msfDialogAnimations} from "./dilog-animations";
 ...
 * @docs-private
 */
export function throwMsfDialogContentAlreadyAttachedError() {
  throw Error('Attempting to attach dialog content after content is already attached');
}

@Component({
  template: "<ng-template cdkPortalOutlet></ng-template> ed",

  selector: 'msf-dialog-container',
  encapsulation: ViewEncapsulation.None,
  // Using OnPush for dialogs caused some G3 sync issues. Disabled until we can track them down.
  // tslint:disable-next-line:validate-decorators
  changeDetection: ChangeDetectionStrategy.Default,
  animations: [msfDialogAnimations.dialogContainer],
...
export class MsfDialogContainer extends BasePortalOutlet{
   ...
   * @param _document
   * @param _options The dialog configuration.
   */
  constructor(
    ...
    public _options: MsfDialogOptions) {
  ...
  attachComponentPortal<C>(portal: ComponentPortal<C>): ComponentRef<C> {
    if (this._portalOutlet.hasAttached()) {
      throwMsfDialogContentAlreadyAttachedError();
    }
